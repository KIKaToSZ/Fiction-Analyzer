...
- **导入 json**：在"打开文件"弹窗里选 `.json`（也会作为日常工作文件）
...
2. 用 FileSystemFileHandle 的 `createWritable()` 把整份 state 写入 `原文件名.json`（轻量序列化，无 SheetJS 开销）
...
**首次保存**时浏览器会弹一次「允许写入」的权限提示。**已授权后**每次保存都是静默写 json。

**导入 json 工作流**：点顶部文件按钮选 `.json`（会覆盖当前数据），后续日常保存会把这份 json 写回原位置；想恢复 xlsx 模式时，导一次 xlsx 即可。
