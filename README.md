...
- **写入文件**：编辑器"保存"按钮直接把修改写回原 xlsx（通过 FileSystemFileHandle.createWritable）；无写入权限时降级为下载 `原名-updated.xlsx`
...
- **导入表格数据**：拖拽 / 选择 xlsx（多 sheet 时可下拉选要导入哪个），或粘贴文本（Tab / 逗号 / 多空格分隔）
...
编辑器里的「保存」按钮：
1. 先把当前章节的修改同步到 localStorage（即时生效）
2. 然后用 FileSystemFileHandle 的 `createWritable()` 把整个 xlsx 写回原文件 —— 含全部 sheet：当前激活的 sheet 用最新章节替换；其他章节 sheet 同步最新数据；非章节 sheet 原样保留

**首次保存**时浏览器会弹一次「允许写入」的权限提示。**已授权后**每次保存都是静默写入原文件。

如果当前没有可写权限（handle 失效 / 旧迁移数据 / 拖入的临时文件），会自动改为下载 `{原文件名}-updated.xlsx`，提示你把更新版放回原路径。
